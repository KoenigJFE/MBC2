...
ss.mem4 <- subset(x = ss.mem2, idents = unique(ss.mem2@meta.data$Specific3)[c(1,4:6)])
ss.mem4$Specific4 <- ss.mem3$Specific

ss.iso <- ss.mem4@assays$RNA@data[c("IGHM", "IGHD", "IGHG1", "IGHG2", "IGHG3", "IGHG4", "IGHA1", "IGHA2"),]
rownames(ss.iso)[apply(ss.iso, which.max, MARGIN = 2)]
ss.mem3$Specific
dt <- as.data.table(cbind(ss.mem3$Specific,rownames(ss.iso)[apply(ss.iso, which.max, MARGIN = 2)]))
colnames(dt)<- c("Specific", "Isotype")

dt <- count(dt, Specific, Isotype)
dt <- merge(dt, ss.mem3[,.N,by = Specific], by = "Specific")
dt$P <- dt$n/dt$N*100

dt$Isotype <- factor(dt$Isotype,levels = c("IGHA2","IGHA1","IGHG4","IGHG3","IGHG1", "IGHM"))
...
#Output
ggplot(dt, aes(P, Specific, fill = Isotype))+
...
ggsave("E:/USER DATA/Koenig/20221112 Submission Copies/Figure 3/Figure 3H.png", height = 2, width = 3.75)
ggsave("E:/USER DATA/Koenig/20221112 Submission Copies/Figure 3/Figure 3H.eps", height = 2, width = 3.75)


#Figure 4 ----
#Figure 4B ----
#This definitely needs love.
